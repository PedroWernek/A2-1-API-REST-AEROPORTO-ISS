...
public class AeronaveHandler implements HttpHandler {
    ...
    private void listar(HttpExchange exchange) throws Exception {
        ...
        String json = mapper.writeValueAsString(aeronaves);//escrevendo a lista de Aeronaves como um json

        enviar(exchange, json); //<- *
    }

    /** Criando um Aeronave no banco
     *
     *
     */
    private void criar(HttpExchange exchange) throws Exception {
        Aeronave p = mapper.readValue(exchange.getRequestBody(), Aeronave.class);
        //pegando o corpo da request que está em json
        //e transformando na classe Aeronave
        //para que assim seja possível a função no repository
        //ler os valores de Aeronave e mandar para o banco

        Aeronave criado = service.criarAeronave(p);//salvando no banco
        String json = mapper.writeValueAsString(criado);//< por isso a função do repository retorna um Aeronave
